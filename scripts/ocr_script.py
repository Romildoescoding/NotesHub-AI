...
def extract_text(image_path):
    ...
    for bbox, text, _ in result:
        if text:
            extracted_text.append({"text": text, "bbox": bbox})
    return extracted_text
...
def clean_text(extracted_text):
    ...
    for item in extracted_text:
        corrected_text = " ".join(
            spell.correction(word) or word for word in item['text'].split()
        )
        item["text"] = corrected_text
        cleaned_data.append(item)
    return cleaned_data
...
def post_process_to_string(ocr_output):
    ...
    for item in sorted_ocr:
        if not current_line or abs(avg_y(item['bbox']) - avg_y(current_line[-1]['bbox'])) < line_threshold:
            current_line.append(item)
        else:
            lines.append(current_line)
            current_line = [item]
    ...
    for line in lines:
        sorted_line = sorted(line, key=lambda item: item['bbox'][0][0])
        line_text = " ".join(item['text'] for item in sorted_line if item['text'])
        multiline_string += line_text + "\n"
